...
top_songs_by_artist <- spotify_all |>
...
```{r}
highest_freq_song <- drake_one_dance |>
...
ggplot(top_artists, aes(x = reorder(artist_name, pos), y = pos)) +
  geom_bar()

```
#the relationship between artist name and position on a playlist is that the more popular the artist the higher the position is. 
...
pid <- c(0 : 999998)
playlist_name <- c("Throwbacks : ✝️")
artist <- c (Missy Elliott : Caleb and Kelsey)
n_artist <- c(0 : 10)
...
positive_contributons <- climate_finance  |>
  climate_finance$purpose == 'adaption'  |> 
  climate_finance$year>= 2011  |>
  climate_finance$year <= 2018

party_counts <- aggregate(party ~ year, data = postive_contributions, FUN = function((x) length(unique(x)))
...
yearly_trends <- (aggregate(contributon ~ year + energy + status, data = climate_fiance, FUN = sum)
                  
ggplot(yearly_trends, aes(x = year, y= contributions, fill = staues)) + 
...
mitigation_projects <- climate_finance

total_contributions <- 
  aggregate(contributons ~ party + year, data = mitigation_projects, FUN = sum)
